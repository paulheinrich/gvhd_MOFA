---
title: "MEFISTO on longitudinal allo-SCT microbiome data"
subtitle: "MEFISTO model which includes metabolites, plots for manuscript"
...
here::i_am("analysis/hep62703_2023-04-28_mefisto/reports/plots_manuscript.Rmd")
...
# Time-covariation analysis

```{r time_covariation_analysis, echo=FALSE}

plot_factors_vs_cov(model, color_by = "time")

plot_factors_vs_cov(model, factors = 1, color_by = "time")
plot_factors_vs_cov(model, factors = 3, color_by = "time")
plot_factors_vs_cov(model, factors = 4, color_by = "time")

print("Scale values representing the time-dependency of a given factor")

covariate_scales <- get_scales(model)
covariate_scales_table <- data.frame(
    "Factor" = names(covariate_scales),
    "Scale_param" = covariate_scales,
    row.names = NULL
)

kable(covariate_scales_table)
...
# Save covariate scale parameter table

write.csv(
    covariate_scales_table,
    file = file.path(table_path, "manuscript", "covariate_analysis", "covariate_scale_parameters.csv"),
    quote = FALSE,
    row.names = FALSE
)
