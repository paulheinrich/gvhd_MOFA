...
here::i_am("analysis/hep62703_2023-04-28_mefisto/scripts/fit_mofa_metabolites.R")
...
# Create time coviariate in metadata
...
metadata$time <- sapply(metadata$amp_Timepoint, function(x) time_mapping[x])

samples_metadata(model) <- metadata

model <- set_covariates(model, covariates = "time")
...
mefisto_opts <- get_default_mefisto_options(model)
...
model_path <- here(
    "analysis/hep62703_2023-04-28_mefisto/data/models",
